...
while [ "$#" -ne 0 ]; do
  case "$1" in
    "-f" | "--force")
      ...
      ;;
    *)
      echo "Usage: download_fw.sh [options]"
...
if [ -f "$ODIN_DIR/${MODEL}_${REGION}/.downloaded" ]; then
  [ -z "$(GET_ANDROID15_FIRMWARE)" ] && exit 0
  if [[ "$(GET_ANDROID15_FIRMWARE)" != "$(cat "$ODIN_DIR/${MODEL}_${REGION}/.downloaded")" ]]; then
    if $FORCE; then
